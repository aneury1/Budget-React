...
const [budgets, setBudgets] =  useLocalStorage("budgets",[]) ///useState([])
const [expenses, setExpenses] = useLocalStorage("expenses",[])///useState([])
...
function deleteBudget(id,{name}){
    ...
    ///const clean  =budgets.map(budget=> budget.name!==name) 
    ///setBudgets(clean);
}
